...
module.exports = class CleanCommand extends Command {
	constructor(client) {
		super(client, {
			...
			args: [
				{
					key: 'limit',
					prompt: 'how many messages would you like to delete? (MAX: 99)\n',
					type: 'integer',
					max: 99
				},
				{
	...
	hasPermission(msg) {
		return msg.member ? msg.member.hasPermission("MANAGE_MESSAGES") : false
	}
	...
	async run(msg, { filter, limit, member }) {
		...
		if (filter) {
			...
			await console.log("❗ " + msg.author.username + " has cleared " + limit + " message(s) in channel: \"" + msg.channel.name + "\"");
